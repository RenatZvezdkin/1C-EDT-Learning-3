Процедура ОбработкаПроведения(Отказ,Режим)
	...
	Если Режим = РежимПроведенияДокумента.Оперативный Тогда
		...
		Запрос.Текст =
			...
			|	ОстаткиТоваровОстатки.ПродажиНоменклатурыОстаток КАК ПродажиНоменклатуры
			|ИЗ
			...
			|ГДЕ
			|	ОстаткиТоваровОстатки.ПродажиНоменклатурыОстаток < 0";
		...
		Если не РезультатЗапроса.Пустой() Тогда
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				Сообщить("Недостаточно товара " + ВыборкаДетальныеЗаписи.Номенклатура +". Нехватает " +(-1*ВыборкаДетальныеЗаписи.ПродажиНоменклатуры)+" единиц.");
	...
	Если не Отказ тогда
			...
			Запрос.Текст =
				...
				|	СебестоимостьОстатки.Товар
				|ИЗ
				|	РегистрНакопления.Себестоимость.Остатки(&ТекущийМомент, Товар В
				|		(ВЫБРАТЬ
				|			РеализацияТоваровТовары.Товар
				|		ИЗ
				|			Документ.РеализацияТоваров.Товары КАК РеализацияТоваровТовары
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				Если не ВыборкаДетальныеЗаписи.Количество=0 тогда
					Себестоимость = ВыборкаДетальныеЗаписи.Сумма/ВыборкаДетальныеЗаписи.Количество;
				Иначе 
					Себестоимость = 0;
				КонецЕсли;
				
				ПеременнаяТовара=Товары.Найти(ВыборкаДетальныеЗаписи.Товар, "Товар");
				Движение = Движения.Себестоимость.Добавить();
				
				Движение.Период = Дата;
				Движение.ВидДвижения = ВидДвиженияНакопления.Расход;
				Движение.Количество = ПеременнаяТовара.Количество;
				Движение.Сумма = ПеременнаяТовара.Количество*Себестоимость;
				...
				Движение.Период = Дата;
				Движение.Количество = ПеременнаяТовара.Количество;
				Движение.Сумма = ПеременнаяТовара.Итого;
				Движение.Номенклатура = ВыборкаДетальныеЗаписи.Товар;
				Движение.Контрагент = Клиент;
				Движение.Себестоимость = ПеременнаяТовара.Количество * Себестоимость;
					
				ИтоговаяСебестоимость = ИтоговаяСебестоимость + Себестоимость;
	...
	Движение.Сумма = ИтоговаяСебестоимость;
